...
def rating_evolution_over_time(df, df_name, bucket=rating_buckets, min_ratings=1000, colors=colors_rating_distribution):
    ...
    df_cleaned['rating_buckets'] = pd.cut(df_cleaned['rating'], bins=bucket, right=False, include_lowest=True)
    ...
    percentage_df = grouped.groupby(level=0).apply(lambda x: x / x.sum()).unstack(fill_value=0)
    ...
    ratings_count_filtered.plot(kind='line', ax=ax2, color='black', marker='o', linewidth=2, label='Total Ratings')
...
def rating_evolution_with_rating_number(df, df_name, colors=colors_rating_distribution, bucket=rating_buckets, nr_reviews=300):
    ...
    df_filtered['rating_buckets'] = pd.cut(df_filtered['rating'], bins=bucket, right=False, include_lowest=True)
    df_amount = df_filtered.groupby(['rating_order', 'rating_buckets'], observed=False).size().reset_index(name='count')
    df_amount['percentage'] = df_amount.groupby('rating_order')['count'].transform(lambda x: x / x.sum())

    # Pivots the data for stacked bar plot
    pivot_df = df_amount.pivot(index='rating_order', columns='rating_buckets', values='percentage').fillna(0)
    ...
    custom_legend_labels = [f"{bucket[i]} - {bucket[i+1]}" for i in range(len(bucket)-1)]
